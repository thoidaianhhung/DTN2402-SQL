...
DROP PROCEDURE IF EXISTS sp_03_01;
DELIMITER $$
...
BEGIN
	WITH c3 AS (
		...
		AND YEAR(CURRENT_DATE()) = YEAR(created_date)
    )
...
DELIMITER $$
CREATE FUNCTION fn_05 () RETURNS ENUM("Essay", "Mulltiple-choice")
BEGIN
	DECLARE v_type_name ENUM("Essay", "Mulltiple-choice");
...
BEGIN
    ...
    UNION
	SELECT 'account' AS type, username AS nmae
    FROM account
...
CALL sp_07 ("Nguyễn Văn Hùng", "1danhvaythoi@gmail.com");

-- Question 8: Viết 1 store cho phép người dùng nhập vào Essay hoặc Multiple-
-- Choice
...
BEGIN
    ...
    SELECT CONCAT("Số lương bản ghi bị xóa: ", v_removed_exam + v_removed_exam_question) AS message;
END $$
DELIMITER ;
...
BEGIN
	DECLARE v_from_departmet_id INT;
    DECLARE v_to_department_id INT;
    
    SELECT department_id INTO v_from_departmet_id
    FROM department
    WHERE department_name = in_department_name;
    ...
    WHERE department_id = v_from_departmet_id;
...
BEGIN
	WITH RECURSIVE c1 (month) AS (
		...
		SELECT question.*, MONTH(created_date) AS month
        FROM question
...
CALL sp_12();
-- Question 13: Viết store để in ra mỗi tháng có bao nhiêu câu hỏi được tạo
-- trong 6 tháng gần đây nhất
-- (Nếu tháng nào không có thì sẽ in ra là &quot;không có câu hỏi nào trong tháng&quot;)

SELECT 	YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AS YEAR,
		MONTH(CURRENT_DATE - INTERVAL 1 MONTH) AS YEAR;
...
BEGIN
    ...
    SELECT year, month,
		IF ( COUNT(question_id) = 0, "Không có câu hỏi nào trong tháng", COUNT(question_id) ) AS question_count
    FROM c2
