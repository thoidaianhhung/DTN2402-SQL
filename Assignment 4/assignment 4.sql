...
-- Question 2: tên phòng ban, tên nhân viên, tên chức vụ
SELECT *
FROM account
...
LIMIT 1;
INSERT INTO `assignment_04`.`exam_question` (`exam_id`, `question_id`)
 VALUES ('2', '1');
 SELECT question.*, COUNT(exam_id) AS count_exam
 FROM question
 LEFT JOIN exam_question USING(question_id)
 GROUP BY question_id;
-- SUPQUERY
...
LIMIT 1;
SELECT COUNT(answer_id) AS count_answer
FROM question
...
LIMIT 1;
-- SUBQUERY
SELECT position.*, COUNT(position_id) AS count_position
FROM position 
LEFT JOIN account USING(position_id)
GROUP BY position_id;
SELECT position.*
...
HAVING COUNT(position_id) = 
	(SELECT MAX(count_position)
    FROM (
    SELECT COUNT(position_id) AS count_position
	FROM position 
	LEFT JOIN account USING(position_id)
...
SELECT *
FROM c11 
WHERE count_position = 
	(SELECT MAX(count_position)
    FROM c11);

-- Question 11: Thống kê mỗi phòng ban có bao nhiêu dev, test, scrum master, PM
SELECT department_id, position_name, COUNT(account_id) AS count_account
FROM department
CROSS JOIN position
LEFT JOIN account USING(department_id, position_id)
GROUP BY department_id, position_id;

-- Question 12: Thông tin question, loại câu hỏi, ai là người tạo ra câu hỏi, câu trả lời là gì
SELECT question.*, category_name, type_name, answer.*
FROM question
LEFT JOIN category_question USING(category_id)
LEFT JOIN type_question USING(type_id)
LEFT JOIN answer USING(question_id);

-- Question 13: Lấy ra số lượng của mỗi loại câu hỏi
SELECT type_id, type_name, COUNT(question_id) AS count_question
FROM question
RIGHT JOIN type_question USING(type_id)
GROUP BY type_id;

-- Question 14: Lấy ra group không có account
SELECT `gourp`.*
FROM `gorup`
LEFT JOIN group_account USING(gorup_id)
...
WHERE group_account.group_id IS NULL;
